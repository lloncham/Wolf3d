...
void	load_xpm(t_wolf *ptr, int bpp, int size, int endian)
{
	...
	ptr->t_size[3] = size;
	ptr->text[2]= mlx_xpm_file_to_image(ptr->mlx, "../xpm/plafond3.xpm", &lar, &lon);
	ptr->text_data[2] = (int *)mlx_get_data_addr(ptr->text[2], &bpp, &size, &endian);
	ptr->t_size[2] = size;
	ptr->text[1]= mlx_xpm_file_to_image(ptr->mlx, "../xpm/sol.xpm", &lar, &lon);
	ptr->text_data[1] = (int *)mlx_get_data_addr(ptr->text[1], &bpp, &size, &endian);
	ptr->t_size[1] = size;
	ptr->text[0] = mlx_xpm_file_to_image(ptr->mlx, "../xpm/mur3.xpm", &lar, &lon);
	printf("lar %d,lon %d\n", lar, lon);
